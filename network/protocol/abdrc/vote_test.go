...
func Test_VoteMsg_Verify(t *testing.T) {
	...
	require.NoError(t, validVoteMsg(t).Verify(tbs), "expected validVoteMsg to return valid vote struct")

	t.Run("VoteInfo is missing", func(t *testing.T) {
		vi := validVoteMsg(t)
		vi.VoteInfo = nil
		require.EqualError(t, vi.Verify(tbs), `vote from '1' is missing vote info`)
	})

	t.Run("invalid vote info", func(t *testing.T) {
		vi := validVoteMsg(t)
		vi.VoteInfo.RoundNumber = 0
		require.EqualError(t, vi.Verify(tbs), `vote from '1' vote info error: round number is not assigned`)
	})

	t.Run("missing US", func(t *testing.T) {
		vi := validVoteMsg(t)
		vi.LedgerCommitInfo = nil
		require.EqualError(t, vi.Verify(tbs), `vote from '1' ledger commit info (unicity seal) is missing`)
	})

	t.Run("invalid vote info hash", func(t *testing.T) {
		vi := validVoteMsg(t)
		vi.VoteInfo.Epoch += 1
		require.ErrorContains(t, vi.Verify(tbs), `vote from '1' vote info hash does not match hash in commit info`)

		vi.VoteInfo.Epoch -= 1
		vi.LedgerCommitInfo.PreviousHash = nil
		require.EqualError(t, vi.Verify(tbs), `vote from '1' vote info hash does not match hash in commit info`)
	})

	t.Run("high QC is missing", func(t *testing.T) {
		vi := validVoteMsg(t)
		vi.HighQc = nil
		require.EqualError(t, vi.Verify(tbs), `vote from '1' high QC is nil`)
	})
	...
	t.Run("unassigned author", func(t *testing.T) {
		vi := validVoteMsg(t)
		vi.Author = ""
		require.EqualError(t, vi.Verify(tbs), `author is missing`)
	})

	t.Run("missing author", func(t *testing.T) {
		vi := validVoteMsg(t)
		vi.Author = "unknown"
		require.EqualError(t, vi.Verify(tbs), `vote from 'unknown' signature verification error: author 'unknown' is not part of the trust base`)
	})

	t.Run("invalid signature", func(t *testing.T) {
		vi := validVoteMsg(t)
		vi.Signature[0] = vi.Signature[0] + 1
		require.EqualError(t, vi.Verify(tbs), `vote from '1' signature verification error: verify bytes failed: verification failed`)

		vi.Signature = []byte{0, 1, 2, 3, 4}
		require.EqualError(t, vi.Verify(tbs), `vote from '1' signature verification error: verify bytes failed: signature length is 5 b (expected 64 b)`)
